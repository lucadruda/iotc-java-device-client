...
public class SSLContextBuilder {
    ...
    /**
     * Create an SSLContext instance with the provided public and private keys that
     * also trusts the base iot hub certificates
     * 
     * @param publicKeyCertificateString the public key to use for x509
     *                                   authentication. Does not need to include
     *                                   the default Iot Hub trusted certificate as
     *                                   it will be added automatically
     * @param privateKeyString           The private key to use for x509
     *                                   authentication
     * @return The created SSLContext that uses the provided public key and private
     *         key
     * @throws GeneralSecurityException If the certificate creation fails, or if the
     *                                  SSLContext creation using those certificates
     *                                  fails
     * @throws IOException              If the certificates cannot be read
     */
    public static SSLContext buildSSLContext(String publicKeyCertificateString, String privateKeyString)
    ...
    /**
     * Build the default SSLContext. Trusts the iot hub base certificates, but can
     * only be used for sas auth
     * 
     * @return the default SSLContext
     * @throws NoSuchAlgorithmException If the SSLContext cannot be created because
     *                                  of a missing algorithm
    ...
    public static RSAPrivateKey parsePrivateKeyString(String privateKeyPEM)
        ...
        if (privateKeyPEM.contains("-----BEGIN RSA PRIVATE KEY-----\n")) {
            return readPrivateKeyPKCS1PEM(privateKeyPEM);
        } else {
    ...
    public static X509Certificate[] parsePublicKeyCertificateString(String pemString) throws GeneralSecurityException {
        ...
        try {
            ...
            throw new GeneralSecurityException("Unable to generate x509 certificate from public key certificate string",
                    e);
        }
    ...
    private static char[] generateTemporaryPassword() {
        return new String("AzureIoTCJava").toCharArray();
    }
